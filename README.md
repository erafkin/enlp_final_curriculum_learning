...
- **Syntactic**: In order to split the data into curricula based on syntactic complexity run `data_processing/syntactic.py`. You would probably need to install some python libraries like spacy before running the program if they aren't installed already, but the code will run as intended once they are available. 
